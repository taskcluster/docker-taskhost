...
/**
Register system in an idempotent way such that we don't fall over if the name
has been registered in the past.

@param {Object} client for papertrail.
@param {Object} config for register system.
*/
function* idempotentRegisterSystem(client, config) {
  var name = config.system.name;

  var systems = (yield client.listSystems()).filter(function(system) {
    return system.name === name;
  });

  if (systems.length) return systems[0];
  return yield client.registerSystem(config);
}


function Papertrail(papertrail, host, systemName, groups) {
  this.host = host;
...
Papertrail.prototype = {
  ...
  */
  setup: function* () {
    ...
    this.system = yield idempotentRegisterSystem(this.client, {
      system: {
        name: this.systemName,
        // Host is required because we deny access from unknown systems...
