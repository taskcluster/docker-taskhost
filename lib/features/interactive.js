...
export default class WebsocketServer {
  ...
  async link(task) {
    await Promise.denodeify(fs.open)(this.lock, 'w');

    debug(path.join(__dirname, '../../bin_utils'));

    return {
      binds: [{
        source: path.join(__dirname, '../../bin_utils'),
        target: '/tmp/.taskcluster_utils',
        readOnly: true
      }, {
        source: this.lock,
  ...
  async started(task) {
    ...
    this.server = new DockerExecServer({
      ...
      path: this.path,
      // wrapperCommand: '/.taskcluster_utils/interactive_wrapper_client.sh ',
    });
