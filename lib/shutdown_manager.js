...
function ShutdownManager(host, runtime) {
  this.host = host;
...
ShutdownManager.prototype = {
  ...
  */
  nextShutdownTime: function* () {
    var shutdownStartRange = this.runtime.shutdownSecondsStart;
    var shutdownStopRange = this.runtime.shutdownSecondsStop;
  ...
  onIdle: co(function* () {
    ...
    this.runtime.log('pending shutdown', {
      time: shutdownTime
    });
  ...
  onWorking: co(function* () {
    if (this.idleTimeout !== null) {
      this.runtime.log('cancel pending shutdown');
