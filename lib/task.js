...
export default class Task {
  ...
  */
  ...
  async dockerConfig(linkInfo) {
    ...
    let commandStr = config.command.map(word => {return word+' ';})
    ...
    let procConfig = {
      ...
      create: {
        Image: config.image,
        // Maybe this could be done in a more general method, but I don't think we'll ever
        // need command injection anywhere else
        // In fact, if we did, this could quickly become a nightmare. How would we combine multiple commands?
        // But this hack will do for now.
        Cmd: config.features.interactive ?
          //run command and store its error code (maybe make it more robust by not using ECODE, and not using sh)
          ['sh', '-c', commandStr + '; ECODE=$?;' +
          //wait for time to expire
            'sleep ' + this.runtime.interactive.expiration +
          //wait for locks to disappear
