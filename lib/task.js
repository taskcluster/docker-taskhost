...
function Task(runtime, runId, task, status) {
  ...
  // Logging handlers will use this to upload logs.
  this.logs = {
    // Primary task container
    task: {
      // TODO: This really should be named something else.
      alias: 'live',
      stream: this.stream
    }
  }
...
Task.prototype = {
  ...
  /**
  Register an additional log stream with the task.
  */
  registerLog: function(name, alias, stream) {
    if (this.logs[name]) {
      throw new Error('Attempted override of existing log: ' + name);
    }
    this.logs[name] = { alias: alias, stream: stream };
  },
  ...
  */
  ...
  pullDockerImage: function* (image) {
    var dockerImage = new DockerImage(image);
  ...
  pullDockerImages: function* () {
    var payload = this.task.payload;
    var images = [payload.image];
    var imageTasks = [this.pullDockerImage(payload.image)];

    if (payload.services) {
      payload.services.forEach(function(service) {
        imageTasks.push(this.pullDockerImage(service.image));
    ...
    // Mark the images so they can be garbage collected later...
    images.forEach(function(image) {
      this.runtime.gc.markImage(image);
    }, this);
  },
  ...
  */
  run: function* () {
    ...
    if (payloadErrors.length) {
      ...
      // If we abort due to schema errors ensure to go through all the states...
      yield* stats.timeGen(
        'tasks.time.states.validation_failed.linked', this.states.link(this)
      );

      yield stats.timeGen(
        'tasks.time.states.validation_failed.created', this.states.created(this)
      );
    ...
    try {
      yield this.pullDockerImages();
    } catch (e) {
      this.stream.write(this.fmtLog(IMAGE_ERROR, e));
      ...
      yield* stats.timeGen(
        'tasks.time.states.pull_failed.linked', this.states.link(this)
      );

      yield stats.timeGen(
        'tasks.time.states.pull_failed.created', this.states.created(this)
      );
