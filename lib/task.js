  ...
  */
  ...
  async dockerConfig(linkInfo) {
    ...
    if(this.task.payload.features.interactive) {
      //only tested with undefined
      let oldEntrypoint = (await this.runtime.docker.getImage(config.image).inspect()).Entrypoint;
      debug('old entrypoint is %s', oldEntrypoint);
      if(typeof oldEntrypoint === 'string') {
        oldEntrypoint = [oldEntrypoint];
      } else if(typeof oldEntrypoint === 'undefined') {
        oldEntrypoint = [];
      }
      procConfig.create.Entrypoint = ['/.taskcluster_utils/busybox',
        'sh',
        '-e',
        '/.taskcluster_utils/interactive_wrapper_run.sh']
        .concat(oldEntrypoint);
      debug(procConfig.create.Entrypoint);
    }
