...
export default class SharedFileLock {
   ...
   * 
   * @param lockFile Name of file to be locked. Must already exist and not be exclusively locked.
   */
  ...
  async acquire() {
    if(!this.lockFd) {
      this.lockFd = await mzfs.open(this.lockFile, 'r');
    }
  ...
  async cleanup() {
    try {
      ...
      await mzfs.unlink(this.lockFile);
    } catch (err) {
