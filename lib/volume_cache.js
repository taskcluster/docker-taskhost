...
export default class VolumeCache {
  ...
  */
  async add(cacheName, instancePath) {
    ...
    // format of <cache name>::<instance id>
    var instance = {key: cacheName + this.config.cache.keyDelimiter + instanceId,
      path: instancePath,
      lastUsed: lastUsed
    };
    return instance;
  ...
  async removeCacheVolume(cacheName, instance) {
    var cacheKey = cacheName + this.config.cache.keyDelimiter + instance;
  ...
  */
  async get(cacheName) {
    if (cacheName.indexOf(this.config.cache.keyDelimiter) !== -1) {
      throw new Error('Invalid key name was provided.  Ensure that the cache ' +
        'name does not contain "' + this.config.cache.keyDelimiter + '".');
    }
    ...
    if (!instanceId) {
      ...
      instance = {key: cacheName + this.config.cache.keyDelimiter + instanceId,
  ...
  */
  async set(cacheKey, value) {
    var cacheName = cacheKey.split(this.config.cache.keyDelimiter)[0];
    var instanceId = cacheKey.split(this.config.cache.keyDelimiter)[1];
